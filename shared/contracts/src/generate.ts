...
for(const route of routes){
  ...
  if(route.query){
    const shape=components[route.query] as {properties?:Record<string,unknown>;required?:string[]};
    for(const [name,schema] of Object.entries(shape.properties??{}))parameters.push({in:'query',name,required:shape.required?.includes(name)??false,schema});
  }
  const security=route.authority==='PUBLIC'?[]:route.authority==='STAFF_OR_PRINCIPAL'?[{staffSession:[]},{principalSession:[]},{staffSecureSession:[]},{principalSecureSession:[]}]:route.authority==='STAFF'?[{staffSession:[]},{staffSecureSession:[]}]:route.authority==='PRINCIPAL'?[{principalSession:[]},{principalSecureSession:[]}]:route.authority==='SUPPLIER_LINK'?[{supplierLink:[]}]:[{machineCredential:[]}];
  ...
  const operation={operationId:route.id,security,parameters,responses,'x-orvia-authority':route.authority,...route.request?{'x-orvia-maximum-body-bytes':route.maximum_body_bytes??16384}:{},'x-orvia-capability':route.capability,'x-orvia-implementation':expansionRoutes.some(r=>r.id===route.id)?'IMPLEMENTED_V1_EXPANSION_PENDING_REVIEW':operationsRoutes.some(r=>r.id===route.id)?'IMPLEMENTED_DPDP_OPERATIONS_PENDING_REVIEW':['grc_audit_response_history','list_grc_audits','create_grc_audit','grc_audit_detail','grc_audit_requests','create_grc_audit_request','grc_audit_request_detail','respond_grc_audit_request','review_grc_audit_response','close_grc_audit','grc_evidence_history','grc_treatment_history','list_grc_frameworks','create_grc_framework','list_grc_controls','create_grc_control','list_grc_risks','create_grc_risk','grc_control_detail','grc_risk_detail','submit_grc_evidence','review_grc_evidence','propose_grc_treatment','review_grc_treatment',].includes(route.id)?'IMPLEMENTED_V1_EXPANSION_PENDING_REVIEW':route.id==='health'?'IMPLEMENTED_A00':['session','list_principals','create_principals'].includes(route.id)?'IMPLEMENTED_A01_PENDING_REVIEW':['list_purposes','create_purposes','list_notices','create_notices','list_policies','create_policies','list_systems','create_systems','publish_policy','reauthenticate_policy','create_mapping','list_mappings','control_map','own_consents','own_receipt','own_history','grant','withdraw'].includes(route.id)?'IMPLEMENTED_A02_PENDING_REVIEW':['workflows','workflow','poll_commands','command_receipt'].includes(route.id)?'IMPLEMENTED_A03_PENDING_REVIEW':['evaluate','send'].includes(route.id)?'IMPLEMENTED_A04_PENDING_REVIEW':['reconcile','attest','failures','overview','evidence','export','check_system','capabilities','restrict','read_simulator','read_simulator_receipt'].includes(route.id)?'IMPLEMENTED_A05_PENDING_REVIEW':['start_test','test_run','list_test_runs'].includes(route.id)?'IMPLEMENTED_A06_PENDING_REVIEW':['list_data_assets','data_asset','create_data_asset','tombstone_data_asset','list_activities','create_activity','list_relationships','create_relationship','graph_search','graph_neighbourhood','graph_impact'].includes(route.id)?'IMPLEMENTED_WP04_PENDING_REVIEW':['list_rights_requests','create_rights_request','rights_request','review_identity','scope_request','transition_request','release_response','record_outcome','list_mandates','create_mandate','revoke_mandate'].includes(route.id)?'IMPLEMENTED_WP07_PENDING_REVIEW':['list_constraints','create_constraint','list_holds','create_hold','release_hold','asset_eligibility','retention_decision','retention_outcome','list_retention_outcomes'].includes(route.id)?'IMPLEMENTED_WP15_PENDING_REVIEW':['coverage','list_gaps','derive_gaps','assign_gap','close_gap','gap_guidance'].includes(route.id)?'IMPLEMENTED_WP13_PENDING_REVIEW':['list_processors','create_processor','link_processor_system','processor_standing','record_coordination','list_assessments','create_assessment','complete_assessment','list_findings','create_finding','close_finding'].includes(route.id)?'IMPLEMENTED_WP16_PENDING_REVIEW':['list_incidents','create_incident','incident_assessment','correct_incident','contain_incident','close_incident','transition_notification','list_obligation_rules','create_obligation_rule'].includes(route.id)?'IMPLEMENTED_WP17_PENDING_REVIEW':['list_templates','create_template','list_notification_tasks','create_notification_task','notification_task','record_delivery','escalation_sweep'].includes(route.id)?'IMPLEMENTED_WP11_PENDING_REVIEW':['entitlements','import_licence'].includes(route.id)?'IMPLEMENTED_WP25_PENDING_REVIEW':'CONTRACT_ONLY_PENDING_TICKET',...(route.request?{requestBody:{required:true,content:{'application/json':{schema:ref(route.request),example:body}}}}:{})};
...
const doc={openapi:'3.1.0',info:{title:'ORVIA customer-local synthetic contract',version:CONTRACT_VERSION,description:`Contract ${CONTRACT_VERSION} is pending consolidated Work review; accepted baseline 0.2.1. Examples are labelled synthetic contract fixtures, never runtime responses.`},paths,components:{schemas:components,securitySchemes:{supplierLink:{type:'http',scheme:'bearer',description:'One-time-issued supplier questionnaire token, scoped to one draft assessment, expiring and revocable. Only its SHA-256 digest is stored.'},staffSecureSession:{type:'apiKey',in:'cookie',name:`${AUTH.staff.secure_cookie_prefix}.session_token`,description:'Verified HTTPS rehearsal profile.'},principalSecureSession:{type:'apiKey',in:'cookie',name:`${AUTH.principal.secure_cookie_prefix}.session_token`,description:'Verified HTTPS rehearsal profile.'},staffSession:{type:'apiKey',in:'cookie',name:`${AUTH.staff.cookie_prefix}.session_token`},principalSession:{type:'apiKey',in:'cookie',name:`${AUTH.principal.cookie_prefix}.session_token`},machineCredential:{type:'http',scheme:'bearer',description:'Independent installation/environment-scoped machine authority; never a human session.'}}}};
...
outputs['endpoint-types.ts']='// Generated from canonical routes. Do not edit.\nexport interface EndpointMap {\n'+routes.filter(r=>r.authority!=='MACHINE'&&r.authority!=='SUPPLIER_LINK').map(r=>`  ${r.id}: { request: ${r.request?`import('zod').infer<typeof import('../src/index.ts').schemas.${r.request}>`:'undefined'}; response: import('zod').infer<typeof import('../src/index.ts').schemas.${r.response}> };`).join('\n')+'\n}\n';
...
mkdirSync('shared/contracts/generated',{recursive:true});
for(const [name,bytes] of Object.entries(outputs)){
  const path=`shared/contracts/generated/${name}`;
