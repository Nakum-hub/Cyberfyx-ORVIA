...
To check a downloaded evidence JSON file offline, run `npm run evidence:verify -- <local-evidence.json>`. The command validates the executable evidence schema and recalculates its embedded content digest without printing evidence fields or contacting a service. For a comparison against a digest retained separately at the time of export, add `--expected-digest <sha256>`. A file whose content and embedded digest were both rewritten can pass the first check; the separate trusted reference catches that change. Neither check proves a downstream effect or the identity of the exporter. The verifier has a 16 MiB file limit and exits nonzero on a failed check.
