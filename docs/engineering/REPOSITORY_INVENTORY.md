...
The logical ownership paths already fit the empty application tree. Root manifests/configuration, `packages/contracts`, `db`, `auth`, `domain`, `testing`, `apps/web` server/liveness, `apps/worker` infrastructure probe, `infrastructure`, `policy`, scripts and backend tests are the only code additions. `apps/web/src/app/{layout,page}.tsx` are the two minimal A00 UI bootstrap files; their transfer is recorded in the Codex handoff. No UI design or later-ticket business feature is claimed. Work-owned JSON, Markdown, ADR and state remain unchanged. Their stale A00 label must be consolidated by Work from the handoff.
...
See `A00-LOCAL-DEVELOPMENT.md`, `A00-CONTRACT-PROPOSAL.md`, `A00-dependency-research.md` and `handoffs/codex/A00-e5cdef3.md` for reproducible commands, contract boundaries and the exact review request.
