...
| `tracking/contract_seed.json` | Codex after bootstrap handoff | Seed mirrors the approved semantic contract; canonical executable schemas live in shared/contracts |
...
| `shared/contracts/**`, `backend/domain/**`, `backend/privacy-control/**`, `database/customer/**`, `backend/auth/**`, `backend/authorization/**`, `connectors/**`, `backend/policy-sdk/**`, `shared/testing/**` | Codex | Includes generated contracts/client transport; no frontend-only copy. `backend/privacy-control/**` is the bounded policy/send-admission module (2026-09-19 extraction from `backend/domain/src/processing/`); it depends on `@orvia/domain`'s shared primitives only, never on a specific connector |
| `frontend/src/app/api/**`, `backend/api/**`, auth middleware/server entry points | Codex | Backend request-handler logic (business/machine/session/admin-principals/auth routing) moved out of `frontend/src/server/` (removed 2026-09-19) into `backend/api/**`; `frontend/src/app/api/**` route files stay thin adapters over `@orvia/backend`. UI imports permitted client interfaces only; no secrets bundled client-side |
| `services/worker/**`, `services/agent/**`, `services/synthetic-target/**`, `policy/**`, `infrastructure/**`, `scripts/**`, `docs/engineering/**` | Codex | This includes fault/reset implementation and target allowlists |
| `frontend/src/app/workspace/**`, `frontend/src/app/privacy/**`, client components/styles/assets, `packages/ui/**` | Codex | Excludes any server/auth authority and any dependency manifest |
| `frontend/src/app/layout.tsx` and UI bootstrap entry files | Codex | Transfer explicitly recorded in A00 handoff before B00 starts |
