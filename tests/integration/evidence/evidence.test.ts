...
try{
 ...
 const task=assigned.obligations[0]!;const attestation={statement:'Synthetic operator reports completing the declared manual restriction task.',evidence_record_ids:[manualWithdrawal.receipt.receipt_id],expected_task_version:0};
 ...
 check('assigned manual attestation accepted',(await member.call(`/api/v1/admin/manual-tasks/${task.id}/attest`,attestation,{'idempotency-key':randomUUID()})).status,202);
