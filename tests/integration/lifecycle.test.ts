...
try{
 ...
 for(let cycle=0;cycle<2;cycle++){
  ...
  const since=(await db.query('SELECT clock_timestamp() at')).rows[0].at;
  ...
  let lastObserved:Record<string,unknown>={};let active=false;for(let n=0;n<120;n++){if(child.exitCode!==null)throw new Error('Supervisor exited before readiness');const observed=(await db.query("SELECT EXISTS(SELECT 1 FROM pg_stat_activity WHERE datname=current_database() AND application_name='orvia_worker') worker,EXISTS(SELECT 1 FROM app.audit_events a JOIN app.request_audit r ON r.id=a.request_id WHERE a.operation='machine.poll' AND a.actor_domain='MACHINE' AND r.status=200 AND a.created_at>$1) agent",[since])).rows[0];lastObserved=observed;active=existsSync(resolve(p.directory,'supervisor/run.json'))&&observed.worker&&observed.agent;if(active)break;await new Promise(r=>setTimeout(r,500));}
  readiness.push({cycle,since,last_observed:lastObserved,supervisor_exit_code:child.exitCode});
