...
const source=()=>git('ls-files','--cached','--others','--exclude-standard').split(/\r?\n/).filter(p=>p&&existsSync(p)&&!p.startsWith('handoffs/')&&!p.startsWith('artifacts/')).sort().map(path=>({path,sha256:hash(readFileSync(path))}));
