...
import { z } from 'zod';
import { Id, Scope, Time } from '../../../shared/contracts/src/index.ts';
import type { RuntimeConfig } from './config.ts';
import { SERVER_LIMITS, type Authority } from '../../../database/customer/src/runtime.ts';
...
export function servicePool(config: RuntimeConfig, role: ServiceRole) {
  ...
  return new pg.Pool({host:'127.0.0.1',port:config.postgres_port,database:role.startsWith('orvia_target_')?config.database+'_targets':config.database,user:role,password,max:2,connectionTimeoutMillis:5000,query_timeout:10000,...SERVER_LIMITS,application_name:role});
}
export function machineAuthority(identity: MachineIdentity): Authority {
  ...
  return {actor_domain:'MACHINE',actor_id:parsed.id,scope:parsed.scope,role:parsed.kind,expires_at:parsed.expires_at,capabilities:parsed.kind==='WORKER'?['workflow.execute','operations.execute']:parsed.kind==='AGENT'?['target.execute']:parsed.kind==='OBSERVER'?['target.observe']:['send.admit']};
}
