...
admin_caps := {"configuration.write", "systems.check", "principals.create", "action.reconcile", "manual.attest", "policy.preview", "graph.write", "rights.write", "retention.write", "coverage.manage", "processor.write", "incident.write", "notification.manage", "support.manage", "registry.write", "operations.execute"}
...
authorize if {
  ...
  input.mfa_verified == true
  input.capability in {"policy.publish", "tests.run", "rights.release", "retention.approve", "incident.approve", "licence.manage", "support.approve", "update.approve", "audit.administer", "connection.enable", "restore.release", "registry.sensitive.read", "registry.sensitive.write", "operations.approve", "regulatory.manage", "sdf.manage"} | export_audit_caps
}
