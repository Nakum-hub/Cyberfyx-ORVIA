 ...
 * control tests and escalates overdue compliance issues and assessment findings.
 */
export type RunnerReport = { scope: string; withdrawals_propagated: number; jobs_processed: number; runs_evaluated: number; runs_executed: number; notifications_created: number; control_tests_run: number; compliance_alerts: number; issues_escalated: number; findings_escalated: number; errors: string[] };
...
export function operationsRunner() {
  ...
  async function once(): Promise<RunnerReport[]> {
    ...
    for (const identity of enrollment.identities) {
      ...
      const report: RunnerReport = { scope: identity.scope.environment_id, withdrawals_propagated: 0, jobs_processed: 0, runs_evaluated: 0, runs_executed: 0, notifications_created: 0, control_tests_run: 0, compliance_alerts: 0, issues_escalated: 0, findings_escalated: 0, errors: [] };
