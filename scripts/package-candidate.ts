...
import { CONTRACT_VERSION,COMMAND_SCHEMA_VERSION } from '../shared/contracts/src/index.ts';
...
const containerBuild=await run('docker',['run','--rm','--network','none','--entrypoint','node',image.Id,'-e',"const fs=require('fs');if(fs.existsSync('/app/.local'))process.exit(1);process.stdout.write(fs.readFileSync('/app/frontend/.next/BUILD_ID','utf8').trim())"]);
