...
const profile=loadProfile();if(profile.profile!=='codex-a00'||process.argv[2]!=='confirm:codex-a00')throw new Error('Named codex-a00 network test required');
...
const source=sourceState();const nonce=randomUUID();const network='orvia-codex-a00-canary-'+nonce;const canary=network+'-endpoint';const runtime='orvia-codex-a00-runtime-'+nonce;const privateNetwork=profile.compose_project+'_private';
const artifact='A06-network-observations-'+nonce+'.json';const assertions:Record<string,unknown>[]=[];let networkCreated=false;let canaryCreated=false;let runtimeCreated=false;
...
try{
 ...
 const args=['create','--name',runtime,'--label',`orvia.network-test=${nonce}`,'--network',privateNetwork,'--cap-drop','ALL','--security-opt','no-new-privileges:true','--memory','3g','--env','ORVIA_PROFILE=codex-a00','--env','ORVIA_TASK_ID=A06','--env',`ORVIA_SERVICE_IPS=${JSON.stringify(peers)}`,'--env',`ORVIA_CANARY_IP=${ip}`,'--env',`ORVIA_NETWORK_ARTIFACT=${artifact}`,'--mount',`type=bind,src=${resolve(profile.directory)},dst=/app/.local/profiles/codex-a00`,'--mount',`type=bind,src=${resolve('handoffs/codex/artifacts')},dst=/app/handoffs/codex/artifacts`,'--mount',`type=bind,src=${resolve('infrastructure/runtime-resolv.conf')},dst=/etc/resolv.conf,readonly`,'orvia-local:prototype'];
