 ...
 if(!row){console.log('No pending synthetic test run.');}
  ...
  try{
   ...
   if(run.request.scenario==='TARGET_RESTORE_QUARANTINE'){
    ...
    const command=(targetGeneration:number)=>{const binding={...original.payload.binding,action_id:randomUUID(),scope:{...original.payload.binding.scope,target_generation:targetGeneration}};const approval={...original.payload.approval,decision_id:randomUUID(),approved_plan_digest:digest(binding),decided_at:new Date().toISOString()};return signCommand({...original.payload,command_id:randomUUID(),nonce:randomUUID().replaceAll('-',''),binding,approval,scope_digest:digest(binding.scope),plan_digest:digest(binding),approval_digest:digest(approval),issued_at:new Date().toISOString(),expires_at:new Date(Date.now()+300000).toISOString()},signer);};
