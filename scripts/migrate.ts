...
try{
  ...
  try{
    ...
    for(const file of readdirSync('database/customer/migrations').filter(file=>/^\d{4}_[a-z_]+\.sql$/.test(file)&&(!bootstrapOnly||file==='0000_bootstrap.sql')).sort()){
      const id=file.slice(0,-4); const sql=readFileSync(`database/customer/migrations/${file}`,'utf8');
