...
export function obligationSatisfied(value:unknown, now:Date){
  ...
  return observation!==null && observation.method!=='NONE' && observation.state==='OBSERVED_SATISFIED' && observation.observed_state===observation.desired_state && observation.observed_at!==null && Date.parse(observation.observed_at)<=now.getTime() && observation.fresh_until!==null && Date.parse(observation.fresh_until)>now.getTime();
}
