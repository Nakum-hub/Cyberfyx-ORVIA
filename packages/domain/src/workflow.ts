...
export async function prepareWorkflow(c: Context, id: string, signer: {key:KeyObject;key_id:string;installation_id:string;agent_id:string}) {
 ...
 for(const systemId of planPolicy.system_ids) {
  ...
  if(mapping&&system.connector!=='LEGACY_MANUAL') {
   actionId=randomUUID();
   const binding=S.PlanBinding.parse({workflow_id:id,action_id:actionId,scope:{...c.actor.scope,principal_reference_id:event.principal_id,system_id:systemId,resource_id:mapping.id,target_subject_reference:mapping.target_subject_reference,purpose_id:event.purpose_id,policy_version_id:event.policy_version_id,consent_epoch:Number(event.epoch),target_generation:Number(mapping.target_generation),operation:system.connector==='SYNTHETIC_CRM'?'CRM_REMOVE_MARKETING_MEMBERSHIP':'SIMULATOR_RESTRICT'},capability:'restrict_exact_synthetic_subject',capability_version:'1.0.0',operation_budget:{maximum_records:1,maximum_attempts:3}});
...
export async function observeAction(c: Context, id: string, observer: pg.Pool) {
 ...
 const read=await targetTransaction(observer,c.actor,tx=>tx.query(`SELECT generation,marketing_restricted,changed_at FROM marketing_memberships WHERE tenant_id=$1 AND legal_entity_id=$2 AND environment_id=$3 AND resource_id=$4 AND principal_id=$5 AND purpose_id=$6 AND system_id=$7 AND subject_reference=$8`,[s.tenant_id,s.legal_entity_id,s.environment_id,s.resource_id,s.principal_reference_id,s.purpose_id,s.system_id,s.target_subject_reference]));
 const target=read.rows[0];const current=target&&Number(target.generation)===s.target_generation;const now=new Date();
 ...
 await c.tx.query('INSERT INTO app.observations VALUES($1,$2,$3,$4,$5,$6,$7)',[...scopeValues(c.actor),observation.id,id,row.principal_id,observation]);
...
export async function readWorkflow(c: Context, id: string) {
 ...
 for(const row of rows.rows) {
  ...
  const observations=await c.tx.query(`SELECT observation FROM app.observations WHERE ${predicate} AND action_id=$4 ORDER BY (observation->>'observed_at') NULLS FIRST,id`,[...scope,row.id]);
  actions.push(S.Action.parse({id:row.id,plan:row.binding,execution_state:row.execution_state,attempts:receipts.rows.map(r=>r.receipt),observations:observations.rows.map(r=>r.observation),reconciliations:[]}));
 }
 ...
 for(const row of (await c.tx.query(`SELECT * FROM app.obligations WHERE ${predicate} AND workflow_id=$4 ORDER BY id`,[...scope,id])).rows) {
  ...
  let current=!!aggregate&&aggregate.state==='WITHDRAWN';
  if(action) {
